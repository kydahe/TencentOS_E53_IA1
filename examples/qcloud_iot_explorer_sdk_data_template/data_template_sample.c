 ...
 *
 */
 //note: cloud can control the light on or off. device can control light on or off automately and send a status message to the cloud if the brightness is changed
...
//light on or off
static void light_event_post(void *client){
	int rc;
	uint32_t eflag;
	//uint8_t event_count;
	sEvent *pEventList[1];
...
static void light_power_on(void)
{
    char *info = "light on ";
    //OLED_Clear();
    OLED_ShowString(0, 2, (uint8_t *)info, 16);
...
static int _get_sys_info(void *handle, char *pJsonDoc, size_t sizeOfBuffer)
{
    ...
    DeviceProperty plat_info[] = {
     	{.key = "module_hardinfo", .type = TYPE_TEMPLATE_STRING, .data = "NBIoT"},
...
/*
static void data_report(DeviceProperty *pReportDataList[], int ReportCont, void *client){

            first_time_report_construct(pReportDataList, &ReportCont);
            int rc = IOT_Template_JSON_ConstructReportArray(client, sg_data_report_buffer, sg_data_report_buffersize, ReportCont, pReportDataList);
            if (rc == QCLOUD_RET_SUCCESS) {
...
/* 5s report the property*/
int cycle_report(DeviceProperty *pReportDataList[], Timer *reportTimer, int *pCount)
{
    int i;
...
static void property_state_change(void){
	//Init_E53_ST1();	
	//E53_ST1_Read_Data();
	//sg_ProductData.m_longtitude = (int)E53_ST1_Data.Longitude;
	//sg_ProductData.m_latitude = (int)E53_ST1_Data.Latitude;
		E53_IA1_Read_Data();	
...
int data_template_light_thread(void) {
    ...
    //init log level
	  printf("data_template_light_thread\n");
    ...
    client = IOT_Template_Construct(&init_params);
    if (client != NULL) {
...
///////////////////////////////////
    ...
    _init_data_template();
		...
		//if(sg_ProductData.m_longtitude != 0 || sg_ProductData.m_latitude != 0) beep_power_off();
